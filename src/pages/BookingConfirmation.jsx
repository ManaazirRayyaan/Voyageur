...
function BookingConfirmation() {
  ...
  const customHotelName = metadata.customHotel?.name;
  const customFlightName = metadata.customFlight?.name;
  const customTransportName = metadata.customTransport?.name;
  const customActivities = metadata.customActivities || [];
  const bookingDestination = booking.package?.destination || booking.destination;
  ...
  return (
    <main className="page-shell flex min-h-screen items-center justify-center bg-slate-50 px-4 py-10">
      <section className="section-card w-full max-w-3xl overflow-hidden p-8 text-center lg:p-12">
        ...
        <h1 className="font-display mt-5 text-4xl font-semibold">Your {bookingDestination} journey is confirmed.</h1>
        <section className="section-card mx-auto mt-8 max-w-2xl p-6 text-left">
          ...
          <div className="mt-5 grid gap-4 text-sm text-slate-600 sm:grid-cols-2">
            <div><p className="text-slate-500">Destination</p><strong className="mt-1 block text-slate-900">{bookingDestination}</strong></div>
            ...
            <div><p className="text-slate-500">Hotel</p><strong className="mt-1 block text-slate-900">{booking.hotel?.name || customHotelName || "Assigned later"}</strong></div>
            <div><p className="text-slate-500">Total Paid</p><strong className="mt-1 block text-slate-900">${booking.totalPrice}</strong></div>
            <div><p className="text-slate-500">Flight / Arrival</p><strong className="mt-1 block text-slate-900">{customFlightName || booking.transport?.type || "Assigned later"}</strong></div>
            <div><p className="text-slate-500">Local Transport</p><strong className="mt-1 block text-slate-900">{customTransportName || "Assigned later"}</strong></div>
            <div><p className="text-slate-500">Activities</p><strong className="mt-1 block text-slate-900">{customActivities.length ? customActivities.map((item) => item.name).join(", ") : "Included package experiences"}</strong></div>
          </div>
        </section>
        <div className="mt-8 flex flex-col justify-center gap-4 sm:flex-row">
          ...
          {booking.package?.id ? <Link to={`/packages/${booking.package.id}`} className="btn-secondary rounded-full px-6 py-4 text-sm font-semibold">View Package Details</Link> : null}
