...
function ForgotPassword() {
  ...
  const resetHref = `mailto:support@voyageur.travel?subject=${encodeURIComponent("Password reset request")}&body=${encodeURIComponent(
    `Please help me reset access to my Voyageur account.\n\nEmail: ${email || "your-email@example.com"}`
  )}`;
    ...
    <main className="grid min-h-screen lg:grid-cols-2">
      ...
      <section className="flex items-center justify-center px-6 py-12">
        <div className="section-card w-full max-w-md p-8">
          ...
          <p className="mt-2 text-sm text-slate-500">Request account recovery support without losing bookings, wishlist items, or profile details.</p>
          <input
            type="email"
            placeholder="Email address"
            value={email}
            onChange={(event) => setEmail(event.target.value)}
            className="mt-8 w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none"
          />
          <a href={resetHref} className="btn-primary mt-4 block w-full rounded-2xl px-5 py-3 text-center text-sm font-semibold">
